...
6. Query the resulting remote item identity, owner, app, visibility, metadata, description, tags, dependency graph, previews, preview bytes, and change history before retaining the receipt.
7. Do not subscribe to, reacquire, or launch the published mod after an ordinary publication. The release already uses the immutable staged candidate and impact-selected pre-publication gameplay evidence. Reserve subscribed-copy verification solely for a deliberate change to the release publisher, Steam subscription/install verification, or its Gateway tooling; invoke that exceptional mode explicitly as tooling validation, never as a routine product-release gate.
...
remote title/description/tags/preview/owner/dependency graph and change history, verifies the served
preview bytes, cleans up the exact publisher process, and retains a token-free receipt. Ordinary
publication must not pass `-VerifySubscribedCopyForReleaseToolingChange`. That explicit mode is only
for validating a deliberate change to the release publisher/subscription/install tooling and is not a
product-release acceptance requirement. The publisher durably records creation/submission

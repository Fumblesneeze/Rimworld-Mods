...
10. Put repository and social destinations in the Steam Workshop item's native Links metadata, not in its BBCode description. Every repository release profile declares the exact canonical `github` link `https://github.com/Fumblesneeze/Rimworld-Mods`; preserve unrelated remote link keys, reconcile only declared keys through SteamUGC, and require the authenticated post-submit query to return exactly one declared key/value before accepting publication.

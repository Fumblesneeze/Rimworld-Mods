...
5. Do not assume generated inline images have a stable public URL. Follow the hosting decision recorded in the OpenSpec design; until its unlisted-item experiment is resolved, treat inline-image publication as incomplete.
...
2. Address an existing declared Workshop item only. Reject ID zero/invalid, ownership disagreement, an unexpected remote fingerprint, or any path that could create a new item.

...
9. Personal observation of new or materially changed player-visible behavior through the live view or exact-run before/action/after screenshots; unchanged regression scenarios rely on their deterministic native-action/outcome assertions and an impact map. Use exact-PID FlaUI only when the gateway cannot expose the needed control or view.

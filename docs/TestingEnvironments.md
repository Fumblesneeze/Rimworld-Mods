...
The runner discovers installed/DLC/Workshop package IDs, writes aggregate JSON and JUnit, and retains each group's durable endpoint state and screenshots. Each E2E scenario must assert its admitted native player action and causally related observable result; API success or a synthetic/direct-state assertion alone is not acceptance. The acting agent inspects exact-run native screenshots for new or materially changed player-visible scenarios, while unchanged regressions rely on deterministic assertions plus an impact map proving stable relevant inputs. Use in-game integration tests for settled loader assertions and E2E tests for multi-frame player behavior; do not make one assembly serve both roles.

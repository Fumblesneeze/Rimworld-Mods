...
| `POST /input/may-maximize-window/click` | `may-maximize-window-click --x N --y N [--button left\|right\|middle]` | Process input that may restore, foreground, or maximize RimWorld |
| `POST /input/may-maximize-window/drag` | `may-maximize-window-drag --start-x N --start-y N --end-x N --end-y N` | Interpolated process input that may restore, foreground, or maximize RimWorld |
| `POST /input/may-maximize-window/keys` | `may-maximize-window-keys (--key K\|--text TEXT) [--modifiers Ctrl,Shift]` | Key/text process input that may restore, foreground, or maximize RimWorld |
...
& $gatewayClient may-maximize-window-click --x 1000 --y 100 --manifest $manifest --pid $pidExpected -o json
& $gatewayClient may-maximize-window-keys --key Escape --manifest $manifest --pid $pidExpected -o json
```
...
Typed version-one placement covers designators whose complete meaning is a cell set plus, for `Designator_Place`, an optional `North`, `East`, `South`, or `West` orientation. The option is valid on one-cell placement and ordered cardinal lines; rectangle drags do not accept it. The gateway initializes and configures the exact revalidated native place designator before its own preflight and designation; it does not rotate the resulting Thing afterward. Rotation on another input/designator and rotation of a non-rotatable placing Def fail closed. Cell or line input for a native `Designator_Build` may also name one exact loaded `stuffDefName`; the placing Def must be Stuff-made and accept it. Invalid material requests fail before designation, and the gateway never rewrites the blueprint or resulting Thing. World targeting and mod-defined multi-stage GUI interactions remain unsupported.

The safe native adapters cover ordinary `Command_Action`, `Command_Toggle`, local `Command_Target`, cell-based `Designator` operations, optional exact cardinal direction for native cell/line place designators, and an independent exact Stuff choice for native cell/line build designators. A material can therefore be selected without forcing a rotation on a non-rotatable buildable such as a Core wall. Group-only UI semantics, abilities/verbs/world targets, sliders, custom `GizmoOnGUI`, right-click menus, material-selection dialogs outside this typed build path, and multi-stage targeting are intentionally `unsupported`; use raw C# or process-scoped input, then add a tested typed adapter when that workflow becomes recurring.
...
At runtime, discovery verifies the owner, complete real active order, assembly identity/hash, attribute metadata, and host manifest before byte-loading the test assembly. The execution state machine persists each admitted transition before invoking the next action. Native steps reuse Gateway time, selection, camera, gizmo, float-menu, screenshot-mode, shadow-rendering, screen-local input, and end-of-frame screenshot services. The typed screenshot-mode action changes only RimWorld's own UI-filter flag on the Unity thread, uses no desktop input, and must be paired with guaranteed restoration when used for clean visual evidence. While that native flag is active, the Gateway host passively suppresses only its own unrestricted-execution and test-status boxes; the server, coordinator, and current workflow continue running. The typed shadow-rendering action changes only `DebugViewSettings.drawShadows`, invokes RimWorld's native map-mesh invalidation callback, and is reserved for same-camera supporting segmentation stages; tests restore the prior value in guaranteed cleanup and restore ordinary shadows before any public candidate capture. `GET /end-to-end-tests` returns retryable pending until the initial token-free snapshot is durable, then exposes discovery, the current test/step, and terminal results. Screenshots are persisted in the same session directory and referenced by the corresponding observation step.

`DesignatorSessionActionStep` retains one native designator across bounded begin, rotate-left,
rotate-right, commit, and cancel actions. The Gateway keeps RimWorld minimized, runs the designator's
native selected lifecycle without registering a second stale-pointer update, and renders its own
highlight once at the retained map cell each frame. It invokes native rotation and one-cell
multi-designation without desktop input. Sessions are exclusive and registry-owned; map, selection,
or gizmo drift atomically cancels the candidate so a fresh session can begin immediately.
...
When an exact native `GizmoActionStep` opens one real on-screen `FloatMenu`, the Gateway leases that exact newly opened instance and holds only it against mouse-distance vanishing; retain the menu screenshot before choosing. `CurrentFloatMenuActionStep` accepts only that same sole open window, requires one enabled ordinal visible-label match, passes through its actual colonist-ordering mode, and mirrors native tutorial allow/chosen/notify/close ordering. Missing, replaced, duplicate, disabled, tutorial-rejected, stale, or multiple menus fail closed. Success consumes the lease; the next semantic action and isolation cleanup clear leftovers, restoring the exact still-open leased menu's prior mouse-distance behavior whenever it is released. This typed path uses no desktop input and is appropriate for modded `Command_Action` menus whose choices do not come from `FloatMenuMakerMap`.

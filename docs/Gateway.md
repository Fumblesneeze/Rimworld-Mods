...
The accepted orbital-meal purchase proof is `artifacts\EndToEndRuns\Grouped\20260805T125710849Z`. The exact Core/Harmony/Immersive/Gateway process generated a one-serving Fine meal through `ThingSetMaker_TraderStock`, placed that exact meal and its Poor steel plate in an orbital trader's native stock container, and opened RimWorld's trade dialog. A typed native trade action resolved that exact meal's current `Tradeable`, applied its native count setter plus dialog refresh from `0` to `1`, then invoked the exact `Dialog_Trade` Accept callback. That semantic path contains no Win32 mouse, keyboard, focus, restore, or maximize operation. The retained trade frames visibly show the Fine-meal row change from `0` to `1` and the 20-silver charge. After ordinary game time landed the orbital delivery, the final full-screen frame visibly shows the selected Fine meal on the colony map and its ordinary inspector reading `Bound plates: 1`. The terminal checkpoint identifies the original meal as `MealFine21650`, the original embedded plate as `ImmersiveChefs_Plate21651`, and `samePlate=True`, proving transfer rather than replacement or duplication. Current E2E tests request a minimized launch but deliberately do not inspect or fail on later window-state changes, whether caused by the player or desktop observation tooling. All five exact-group tests passed, the roof-first disposable-map reset produced no collapse/death leakage, and stage/credential/process/config cleanup passed. Settlement trade remains a separate pending acceptance path.

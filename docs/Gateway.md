...
The accepted user-window-change regression is `artifacts\GatewaySmoke\launcher-user-window-change-final\20260803T143327162Z`. The real launcher ran with `-WarningAction Stop`, requested `Minimized`, then the exact owned window was maximized before its native observation. `window-launch-observation.json` records `IsMinimized=false`, `MatchesExpectedWindowStyle=false`, `StyleMismatchIsFatal=false`, and the explicit user-change diagnostic while both RimWorld and the launcher host remained alive. No warning stream was emitted; the same invocation exited zero, completed exact-process and credential cleanup, and preserved the user's normal configuration hashes.

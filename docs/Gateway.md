...
The accepted user-window-change regression is `artifacts\GatewaySmoke\launcher-user-window-change-final\20260803T143327162Z`. The real launcher requested `Minimized`, then the exact owned window was maximized before its native observation while both RimWorld and the launcher host remained alive. That earlier artifact motivated the final simpler contract: the window snapshot is informational only and user changes are neither classified nor enforced.

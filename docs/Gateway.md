...
`scripts\Invoke-GatewaySmoke.ps1` defaults to only Core + Dev Gateway in an isolated run. `-AdditionalModIds` inserts validated, de-duplicated package IDs between Core and the gateway, which lets the same harness load Harmony and a product mod without touching the normal mod list. For product evidence, `-AdditionalModProjectPaths` builds and deploys the exact repository product project before launch and records its assembly/About/XML-patch hashes; the ordinary deployed package is rejected if a test assembly or Gateway integration contract leaked into it. `-ExpectedIntegrationTests` names the exact owner, lifecycle, and fully qualified product test that must be discovered and pass. A non-Gateway expected owner is rejected unless the corresponding repository product project is supplied, so a stale preinstalled package cannot satisfy provenance. The script waits for the exact PID's manifest, reads it through a 64 KiB non-reparse bound, requires its exact active schema and JSON types, safe run ID, exact `http://127.0.0.1:<port>/api/v1` base URL, and matching PID plus process-start instant before using its token or paths. It then proves unauthenticated `401` and authenticated traffic through the real Unity Mono EmbedIO listener, checks all packaged DLLs including `Mono.CSharp.dll`, and submits raw C# that declares persistent state, verifies RimWorld's actual loaded-mod order matches the requested list, inspects live Unity/game state, performs a reversible mutation, and restores it. In Quicktest it also gets/sets/restores developer mode, god mode, pause/speed, and camera; queries, inspects, and atomically selects spawned things; directly spawns and cleans up a disposable item and pawn with idempotent replay; invokes one exact safe native debug action; and exercises a reversible native gizmo plus disposable architect interaction. It captures correlated JSON, API and FlaUI screenshots, checks the Player log, requests controlled shutdown, and verifies the normal mod-list hash is unchanged.

The click probe retries foreground activation. In a non-interactive desktop session Windows may still deny foreground ownership; by default the smoke records the gateway's explicit `focus_lost` safeguard as `ClickOutcome: focus-guard-rejected` rather than injecting into the wrong window. Pass `-RequireRawClick` for a dedicated interactive input run that must inject successfully.

Evidence lives under `artifacts\GatewaySmoke\<UTC run id>` and includes the immutable aggregate `evidence-summary.json`, `SavedData\Config\ModsConfig.xml`, `Player.log`, `dotnet-build.log`, `status.json`, `execution.json`, `execution-state.json`, `execution-restore.json`, `ui-state.json`, `logs.json`, `actions.json`, `automations.json`, the mandatory exact-process `gateway-screenshot.png`, `flaui-evidence.json`, `click.json`, `shutdown.json`, `process-cleanup.json`, `credential-cleanup.json`, `integration-test-stage-cleanup.json`, and aggregate `cleanup-status.json`. `main-menu.png` exists only when the independent FlaUI desktop capture completed; an unavailable FlaUI capture remains explicit and is never replaced with a copy of the Gateway image. The evidence summary records configured and live RimWorld versions separately, the Gateway mod version, and each required deployed DLL's filename, complete managed assembly identity, byte length, and SHA-256. A product run additionally records the exact repository project and package root; the complete ordinary-package relative-path/length/SHA-256 inventory; every managed DLL's full identity and metadata-only reference inventory; the primary product assembly identity/length/SHA-256; About metadata hash; and every source/deployed XML patch relative path/length/hash. Quicktest camera verification additionally retains before/move/zoom/restore JSON, a settled game-state snapshot after each rendered-frame boundary, and `camera-before.png`, `camera-after-move.png`, `camera-after-zoom.png`, and `camera-restored.png`. The isolated `ModsConfig.xml` and command result record the ordered Core/additional/gateway list. Each FlaUI child is time-bounded and terminated only through its retained process handle. Setup records service/connect attempts before invocation; cleanup reconciles observed disconnected/stopped state after uncertain responses and preserves a service that was already running and disconnected. Each cleanup component writes an explicit completed, failed, or not-required/not-requested status, and the aggregate retains primary verification plus process, credential, and stage-cleanup failures together rather than masking later cleanup failures. After the exact launched PID is confirmed dead, including after the force fallback, the host removes a matching `SavedData\DevGateway\current.json`, atomically removes credentials from a matching active `session.json`, and scans every retained run artifact for every observed bearer token. A normal controlled shutdown still leaves the runtime's `stopped` token-free tombstone unchanged.
...
Add `-Quicktest` to wait for a playable map and run the checked-in `scripts\Fixtures\GatewayQuickstartSmoke.json` descriptor twice with one idempotency key:
...
A successful quicktest additionally retains `quickstart.json`, `quickstart-replay.json`, `post-quickstart-status.json`, `post-quickstart-ui-state.json`, and `post-quickstart-logs.json`. The script captures the log cursor before setup, then writes the post-quickstart log page after that cursor and captures fresh status/UI state and the screenshot after the terminal run and replay. The replay must report the original run ID and no duplicate spawned objects. Product-mod runs should pass `-ExpectedLogMarkers` for known initializer success messages; the smoke also rejects common structured mod/gateway exception signatures. If the post log page reports `PageTruncated`, continue manually after its final returned sequence when the full range matters.

...
Before and after every test, the process pauses and removes every constructed or natural roof—including overhead mountain—before destroying any map content, then removes all destroyable spawned Things/Pawns, zones, designations, selection, active interactions, and test-opened windows. This order prevents roof collapse from killing fixtures or leaking alerts into later scenarios. Permanent non-destroyable map features such as quicktest steam geysers remain environment. Cleanup restores control/camera state and verifies that both roofs and the disposable baseline are empty; an unverifiable reset taints the process and skips later tests. The host continues with later independent mod groups, writes aggregate JSON/JUnit, shuts down the exact PID, sanitizes credentials, and removes only the exact leased test stage in `finally`.
...
`immersive-chefs-microwave-reheating` is passive after setup. It creates a hungry drafted colonist, an initially frozen Masterwork-80 plated Simple meal, exact clean steel cutlery, and a real microwave connected to a charged battery power net, then selects the meal and pauses. It never chooses or starts an ingest job and never calls either reheat method. Use the colonist's native Undraft gizmo and ordinary speed control; observe the pawn carry the meal to the appliance, then inspect the Needs panel and returned ware after ingestion. Successful reheating is visibly distinguishable because the configured five-point loss crosses the Masterwork-to-Excellent boundary and the 60 °C output produces `Steaming hot meal`.
...
The earlier grouped run automatically restored and maximized RimWorld, and the orbital-trade test was consequently displayed in that maximized window; this was not a user window change. The external 20 ms exact-PID timeline at `artifacts\EndToEndRuns\WindowLaunchFinal\20260805T161025756Z` shows that the process first left its minimized state immediately after the first fixture held Unity's main thread for 18.5 seconds and was already maximized before the orbital test began. The corrected countertop fixture searches a fixed candidate grid and reaches its first player-action step in 1.45 seconds. The reviewed rerun's credential-free `window-timeline.json` contains exactly one native state for PID 19740 from first observation through shutdown: `ShowCommand=2`, `Iconic=true`, with the outer normal window rectangle `-7,0–1609,939` corresponding to the configured 1600×900 client and no restore or maximize transition. All five exact-group tests passed, the stage and credentials cleaned, and both normal configuration hashes remained unchanged. This timeline is external read-only acceptance evidence, not a launcher monitor; the launcher still never inspects or overrides a later user restore/maximize. Settlement trade remains a separate pending acceptance path.
...
The accepted RimCuisine/No-Vanilla native-cooking proof is `artifacts\EndToEndRuns\Grouped\20260805T154611301Z`. Its one exact minimized process loaded Core, Harmony, Processor Framework, all four RimCuisine 2 modules, No Vanilla Meals, Immersive Chefs, and Gateway. Two Cooking-only colonists used ordinary `DoBill` jobs at fueled stoves: the ordinary UI visibly shows `Thin pottage x5` with `Bound plates: 5` and `Extravagant meal x10` with `Bound plates: 10`, while the latter's ingredient line contains only upstream cassowary egg, agave fruit, corn, and raw fungus. The selected returned cookware in both kitchens visibly reads `Cleanliness: dirty`. The terminal checkpoint independently records the exact `CookThinPottage`/`RC2_CookExtravagantMeal` recipes, `nativeBillObserved=True`, five WoodLog plates versus ten Silver plates, one culinary record per serving, and the rejected ten-stack of Steel plates still spawned beside the extravagant kitchen. The run passed final log scanning and cleaned the exact process, credential, Workshop override, and leased test stage while preserving both normal configuration hashes. Pizza trade and the excluded-product visual inventory remain the unfinished half of this compatibility row.

...
| Dubs Rimatomics | `Dubwise.Rimatomics` | Uranium cookware, plates, and cutlery use Rimatomics radiation sickness instead of Core toxic buildup. Rimatomics keeps control of its strength setting, radiation resistance, robots, and severe exposure. If it is absent or switched off, Crash Landing gets the next chance to handle radiation before the Core fallback. Lead always remains toxic buildup. |
| Crash Landing | `Katavrik.CrashLanding` | When Rimatomics is not handling uranium, uranium cookware, plates, and cutlery add Crash Landing's native `Rad` illness instead of Core toxic buildup. Rimatomics takes priority when both are active; without either provider, uranium uses the Core fallback. Lead always remains toxic buildup. |

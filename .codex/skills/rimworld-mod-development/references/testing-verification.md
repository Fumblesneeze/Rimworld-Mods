...
Run final live acceptance after independent review and all accepted fixes. The acting agent must perform an actual player action—or faithful automation of the same native UI, gizmo, bill, job, designation, hauling, ingestion, construction, selection, or game-command path—and personally observe the resulting player-visible behavior in the running game or exact-run before/action/after screenshots.

Logs, loaded Defs, Harmony patch inspection, API success, gateway JSON, raw-C# reads, and direct state mutation are supporting diagnostics only. They cannot accept gameplay by themselves. Quickstart/direct spawn may prepare disposable fixtures, but must not construct the outcome under test. Retain the exact action sequence and describe the observed result. If review or later edits can affect runtime behavior, discard older acceptance evidence and repeat the run.
...
Inspect Player.log for `FileNotFoundException`, `TypeLoadException`, `MissingMethodException`, patch failures, XML errors, unresolved cross-references, and gateway request failures. A package only passes a third-party-library decision after those DLLs load and serve a real request inside Unity Mono.
...
For Harmony, prove the resolved target signature and that `Harmony.GetPatchInfo(target)` contains the owning Harmony ID for the expected patch exactly once. For XML, inspect the loaded Def database, not only packaged XML. These are supporting checks. In both cases drive the actual cooking scenario through its player-facing bill/job path and personally observe the resulting game behavior; add a scenario-owned quickstart fixture only to arrange preconditions when the stock fixture is insufficient. A semantic/raw-C# state probe may corroborate the observation but cannot replace it. Startup, static screenshots, and absence of log errors are necessary but not sufficient. Record the external mod package ID, version, assembly filename, MVID or SHA-256, and selected integration strategy.

...
- [ ] 14.4 SHOWCASE SCENES: build the four natural Immersive Chefs scenes with the exact optional package chains, believable rooms/materials/decor/names/apparel/storage/linking, compact action geography and ordinary native jobs/orders/memories. Do not reuse cleared-map regression fixtures as publishable visuals.
...
- [ ] 14.5 CAPTURE/ENCODE: personally capture and review one high-detail screenshot plus one no-longer-than-five-second GIF for each showcase, and the requested readable memories screenshot. Use tight fixed crops where the acting pawn, manipulated Things and destination are plainly legible; prefer hard cuts to camera pans or wide colony views, retain source frames/crops/encoder provenance, and make every final additional preview smaller than one MiB.
...
- [ ] 14.6 PRESENTATION/RELEASE: replace or reorder the mod's declared additional previews without repeating its primary preview, generate the local carousel review, update the release and Gateway skills with the proven workflow, run strict OpenSpec/package/release validation, independently review the slice, and prepare the immutable Steam dry-run with its exact authored change note. Do not publish until that new exact dry-run is reviewed under the existing mutation gate.

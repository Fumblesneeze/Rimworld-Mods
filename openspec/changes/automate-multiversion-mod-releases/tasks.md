...
- [ ] 14.4 SHOWCASE SCENES: build the four natural Immersive Chefs scenes with the exact optional package chains, believable rooms/materials/decor/names/apparel/storage/linking, compact action geography and ordinary native jobs/orders/memories. Do not reuse cleared-map regression fixtures as publishable visuals.
  - [x] 14.4a RESEARCH/SKILL: establish a non-guessing showcase-design process; inspect a varied player/mechanics corpus and a bounded read-only 12-blueprint Real Ruins sample, codify source-linked confidence classes, colony/room/archetype rules, untrusted-blueprint safety budgets and a per-showcase design-record template in the repo-local `rimworld-realistic-base-generation` skill, and route the release workflow through it. Raw screenshots, contact sheets and blueprints remain ignored research inputs; weakly attributed visuals are explicitly ineligible for recurrent rules.
...
- [ ] 14.5 CAPTURE/ENCODE: personally capture and review one high-detail screenshot plus one no-longer-than-five-second GIF for each showcase, and the requested readable memories screenshot. Prefer cuts to camera pans, retain source frames/crops/encoder provenance, and make every final additional preview smaller than one MiB.

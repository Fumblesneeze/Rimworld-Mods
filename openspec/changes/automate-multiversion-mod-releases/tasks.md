...
- [ ] 14.1 CONTRACT: add strict per-mod showcase declarations and the exact Immersive Chefs inventory: Gastronomy order-to-service; Pick Up And Haul/Dubs/Processor dirty-dish batch; prisoner nutrient-paste dining in Prisoner Jumpsuits; prepared-food/fine-meal/Thermodynamics hot storage; and a memories still showing vanilla ate-without-table plus Immersive Chefs ate-without-cutlery and cold-food memories.
- [ ] 14.2 CAPTURE: extend the Gateway presentation capture surface with non-selecting Thing crops, camera-centered pixel rectangle/offset crops, bounded frame sequences, exact applied-rectangle/timing evidence, and state restoration; use the proven capture mode in game.
- [ ] 14.3 CHANGE NOTES: make every retained-ID update reject an empty/generic/unchanged note, bind the authored note into dry-run/confirmation, submit it unchanged, and retain its verified Steam change-history result. Keep first publication's note policy separate.

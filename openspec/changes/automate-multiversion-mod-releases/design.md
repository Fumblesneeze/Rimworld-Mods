...
The release CLI resolves a full-install cache entry and passes its exact executable, data path, matching compatibility-folder output, and build identity into the current scenario/grouped-E2E runners. This permits several exact regression builds to exercise the one binary selected for their compatibility line. Each group still declares its entire ordered mod list. The runner launches a fresh exact PID with a unique savedata folder, records the depot/file identities, and asserts the admitted native player action, its causally related observable outcome, and exact-process cleanup. A release impact map selects scenarios affected by runtime code, XML/Defs, assets, dependencies, engine target, workflow, observation contract, or harness changes. The acting agent personally inspects before/action/after screenshots for new or materially changed player-visible scenarios; unchanged scenarios rely on deterministic assertions when the map proves their relevant staged bytes and inputs are identical.
...
The operator authorizes that meaningful two-phase release once. Exact hashes, nonces, and command confirmation strings are machine guardrails, not text the user must repeat. Synchronizing the already reviewed local image bytes and replacing their tokens or prior Steam URLs with byte-verified Steam-assigned URLs remains within that authorization. A committed source-tree hash excludes exactly the generated resolved description and its provenance; an intent hash also binds the item, visibility, title/tags, dependency graph, change note, primary preview, ordered additional-preview hashes and showcase policy. The preview inventory and resolved-description provenance carry both hashes, and final staging refuses lineage drift. The workflow asks again only when those meaningful inputs or requested scope change, or when recovery would require another Steam submission.
...
On success, query the item through SteamUGC and compare item identity, owner, app, visibility, title, description, tags, metadata fingerprint, preview inventory, exact required-item graph, served preview bytes, and published change note with the reviewed bundle. Inspect the staged `About/About.xml` to confirm supported versions and required package dependencies match the same manifest. Record remote discrepancies as a failed verification even when `SubmitItemUpdate` returned success. Ordinary product publication ends after this remote verification and exact-process cleanup; it does not subscribe to, reacquire, or launch the published mod.

Implementation tests use a narrow fake SteamUGC adapter to exercise identities, stale nonces, setter failures, timeouts, late callbacks, and legal-agreement states. Final acceptance of publication requires explicit user authorization and a dedicated unlisted/private test Workshop item; it cannot use a production item or synthetic mutation. Every impacted claimed target needs asserted native player-workflow acceptance before publication. The Dev Gateway is the default controller for that impact-selected gameplay evidence. Subscribed-copy package comparison and gameplay smoke remain a separate explicit validation mode only when the release publisher, Steam subscription/install verification, or related Gateway tooling itself changed; they are not an ordinary product-release gate.

An operator stop or no-launch directive disables that verification profile for the remainder of the release. Cleanup records graceful/exact-PID shutdown and a zero matching-process check. The workflow does not relaunch the disabled profile without fresh explicit authorization, but may continue through a separately authorized minimized Gateway profile.
...
- **[A Gateway publisher weakens product isolation]** → Keep publisher code/package ownership in RimWorld Dev Gateway, validate product package exclusions and managed references, and keep impact-selected gameplay acceptance on the immutable product candidate without making Gateway a product dependency. Use subscribed-copy validation only to test deliberate release-tooling changes.

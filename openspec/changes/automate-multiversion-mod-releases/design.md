...
- Create exactly one first Workshop item only when a manifest explicitly opts into first publication and a dry-run matching the explicit publication order has no item ID; thereafter update only that retained identity and verify the remote result without a redundant post-prepare prompt.
...
On success, query the item through SteamUGC and compare item identity, title, description, tags, metadata fingerprint, preview inventory, and exact required-item graph with the reviewed bundle. Inspect the packaged `About/About.xml` to confirm supported versions and required package dependencies match the same manifest. The immutable plan retains the pre-submit Steam `time_updated` value. Publication completes only after an authenticated query of the same item reports a strictly greater nonzero `time_updated` value together with the expected remote graph. An unchanged timestamp is not success even when `SubmitItemUpdate` returned success.

Implementation tests use a narrow fake SteamUGC adapter to exercise identities, stale nonces, setter failures, timeouts, late callbacks, legal-agreement states, unchanged timestamps, and modified-time convergence. Final acceptance of publication requires explicit user authorization and a real Workshop item; it cannot use a synthetic mutation. Every claimed target still needs its native player-workflow acceptance before release. Gateway-based workflows satisfy that gate without a separate run omitting the Gateway; product independence is checked through dependency metadata, package contents, and assembly references. Those development gates do not run inside or extend the publication transaction.

### 10a. Keep publication generic and leave testing before the release trigger

`release_publish` is a repository-wide Steam publication operation. It consumes only the strict universal release profile and immutable plan; it has no per-mod test ID, cooking/dining workflow, subscriber adapter, screenshot expectation, or local-mod-install behavior. Triggering a release asserts that the candidate's applicable tests, gameplay acceptance, balance review, and presentation review were already completed through their owning development workflows.

The publisher may launch the isolated Core-plus-Gateway process needed to use SteamUGC, submit the exact update, reconcile required-item and presentation metadata, poll the exact item's authenticated remote record, and persist a credential-free receipt. It must not call `SubscribeItem`, wait for Workshop content installation, move or restore the canonical local mod, launch a second gameplay process, run a product-specific smoke, or require a later personal-evidence acceptance operation. A subscribed-copy check remains possible only as a separately and explicitly requested diagnostic workflow outside the release pipeline.

This narrower transaction makes release behavior identical for every mod. It also makes the terminal signal match what the Workshop exposes: the exact item has the expected remote state and its Steam modified time advanced beyond the baseline frozen into the plan.
...
The dry-run shows additions and removals in all three projections and binds the exact graph hash into admission. Because Steam dependency operations are asynchronous and separate from content submission, the workflow does not claim atomicity: a partial result remains unverified, preserves its candidate/remote snapshot, and retries only the unresolved admitted edges after querying remote state. Remote verification must match the exact required graph; optional items appearing as Steam requirements are a failure.
...
- **[RimWorld, description, and Steam dependencies disagree]** → Derive all three from one typed manifest graph, include the graph in dry-run admission, and verify both packaged and remote projections.
...
- **[The release orchestrator becomes a second test framework]** → Keep tests and native acceptance in their owning pre-release workflows; the publisher only validates immutable release inputs and Steam's remote result.
- **[A Gateway publisher weakens product isolation]** → Keep publisher code/package ownership in RimWorld Dev Gateway and verify that product dependency metadata, package contents, and assembly references exclude the Gateway. No separate run without Gateway is required.

...
- **[A Gateway publisher weakens product isolation]** → Keep publisher code/package ownership in RimWorld Dev Gateway, validate product package exclusions and managed references, reacquire the exact subscribed Workshop package, and assert its native workflow through the minimized Gateway controller without making Gateway a product dependency.

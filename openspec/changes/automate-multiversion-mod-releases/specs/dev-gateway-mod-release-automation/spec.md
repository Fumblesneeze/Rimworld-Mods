...
#### Scenario: Immersive Chefs opens its Workshop gallery on its title art
- **WHEN** the Immersive Chefs additional-preview inventory is compiled
- **THEN** the reviewed primary title image occupies additional-preview slot zero, the six feature cards follow in their declared order, and the title image has no BBCode image token
...
- **THEN** the workflow validates the deterministic lineage and continues through final submission and remote verification under the existing authorization without prompting the operator again
...
One cross-process lease SHALL cover identity recovery, mutation admission, remote reconciliation, dependency reconciliation, and receipt creation. Before an ID-less first publication, a bounded native query of every item published by the owning account SHALL prove that no exact-title RimWorld item exists; an incomplete query, one exact-title item, or more than one exact-title item SHALL fail before `CreateItem`. Every async Steam call MUST reject an invalid handle and correlate callback parent/child/item identities with the admitted request. A duplicate-create callback carrying one nonzero identity SHALL be treated as an already-created identity and persisted, not as permission for another create. Steam's legal-agreement flag SHALL stop before submission on creation as well as after submission. Definite callback failures MAY be retried from a fresh reviewed invocation; I/O failure, a timeout after admission, or a completed upload whose remote-verification/receipt phase did not finish is indeterminate and MUST block a different plan until the exact prior plan is reconciled. Existing receipts participate in identity recovery and conflicting identities fail closed. Remote acceptance SHALL require exact title, owner, app, visibility, description hash, metadata, tags, dependency set, published change note, and downloaded preview hash before receipt creation.
...
### Requirement: Ordinary publication does not subscribe or run post-publication gameplay
After Steam reports a successful update and remote propagation, the ordinary release workflow SHALL verify the exact item identity, owner, app, visibility, title, description, metadata, tags, required-item graph, preview inventory/bytes, and published change note, retain its credential-free receipt, clean up the exact publisher process, and stop. It MUST NOT subscribe to or refresh the published item, reacquire it into the Steam Workshop content area, move the local product out of RimWorld discovery, or launch another RimWorld process for a post-publication gameplay smoke. The immutable staged candidate and impact-selected pre-publication native workflows own product acceptance.

Subscribed-copy package comparison and gameplay smoke SHALL remain available only as an explicit tooling-validation mode when the release publisher, Steam subscription/install verification, or related Gateway tooling itself changed. Selecting that exceptional mode SHALL be deliberate and SHALL be recorded in the receipt; it MUST NOT become an ordinary product-release gate.

#### Scenario: Ordinary Immersive Chefs update ends after remote verification
- **WHEN** a confirmed Immersive Chefs product update reaches Steam without a release-tooling-change verification request
- **THEN** the workflow verifies the remote item and change note, writes the receipt, cleans up its exact process, and performs no SubscribeItem call, Workshop package reacquisition, local-product move, or post-publication gameplay launch

#### Scenario: Release tooling change explicitly exercises subscriber verification
- **WHEN** the release publisher, Steam subscription/install verification, or related Gateway tooling changed and the operator explicitly selects the tooling-validation mode
- **THEN** the workflow may subscribe/reacquire the exact item and run the isolated subscribed-copy native smoke, records that exceptional purpose and evidence, and restores local state in guaranteed cleanup

#### Scenario: Exceptional subscribed verification preflights a legacy-safe evidence path
- **WHEN** an explicit release-tooling-change validation prepares the subscribed-copy Gateway smoke on Windows
...
- **THEN** the impact map classifies it as unchanged, its deterministic pre-publication native-action and observable-outcome assertions remain sufficient, and no duplicate post-publication run or manual screenshot review is performed

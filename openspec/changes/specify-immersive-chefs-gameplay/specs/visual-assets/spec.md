...
After the same final-scale resize, `portable-fine` art SHALL have at least `0.95` dark fraction in ring 1 and `0.40` in ring 2, `portable-broad` art SHALL have at least `0.95` in ring 1 and `0.75` in ring 2, and building art SHALL have at least `0.95` in rings 1, 2, and 3. The manifest SHALL classify chef's knives and cutlery as `portable-fine`; cookware, primitive/glitter cookware, plates, and prepared ingredients as `portable-broad`; and every appliance/station cardinal diffuse as `building`. Permitted added exterior expansion is one final pixel for fine art and one or two final pixels for broad/building art.

Before any product diffuse is processed, `docs/SpriteOutlineApprovals.xml` SHALL record its pre-outline hash and final-scale topology oracle: the exact count of 8-connected foreground components and 4-connected background holes whose area is at least two pixels, plus an explicit horizontal or vertical final-scale cross-section for every narrow semantic gap that a selected stroke could close. Each protected cross-section SHALL record its fixed coordinate, inclusive start/end coordinates, and the minimum consecutive run of pixels that must remain below alpha `96`; a single protected point is insufficient. The outlined result SHALL preserve those component/hole counts exactly and every protected transparent run at or above its recorded width. The manifest SHALL record the selected stroke, output hash, post-outline ring fractions, and reviewed evidence. A deterministic processor MAY then apply the selected bounded stroke, but every added contour coordinate MUST have been fully transparent and part of the 4-connected canvas-edge exterior background before outlining; enclosed holes and semantic gaps MUST NOT receive contour. The processor MUST preserve every pre-existing nontransparent source pixel; leave canvas dimensions and Def draw size unchanged; retain at least two fully transparent final-scale pixels between the resulting alpha bounds and every canvas edge; and preserve cardinal equipment identity and sanitation details. It MUST NOT merge distinct cookware/cutlery parts, close utensil tines or inter-part gaps, turn fine handles into blobs, or create an external cast shadow.

For every `CutoutComplex` diffuse, the outlined sibling mask SHALL have byte-for-byte equal alpha. Newly added contour pixels SHALL be fixed black in the mask rather than red/green Stuff regions, so wood, stone, steel, silver, and gold tint only the intended material surfaces while the outline stays dark. Every pre-existing nontransparent mask pixel and material classification SHALL remain byte-identical; formerly transparent pixels are exempt only where the processor adds the fixed-black contour with diffuse-matching alpha. Fixed-color/Cutout sprites SHALL gain no unnecessary mask. Outline candidates and final acceptance SHALL be compared after the same mip/downscale used by the game on both light and dark map contexts; full-resolution source inspection alone is insufficient.

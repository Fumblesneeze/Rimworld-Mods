...
The modern metal/plastic cookware family SHALL use broad low-frequency value groups and smooth curved silhouettes that survive RimWorld's mipmapped map rendering without stippled, checkerboard, or block-fragment artifacts. Its pot, loose lid, pan, and utensil SHALL retain open negative spaces at useful map scale instead of collapsing into an animal-like or scrap-like compact blob. Its measured foreground SHALL be fully opaque, with fractional alpha confined to a narrow antialiased silhouette boundary or an explicitly protected semantic gap; a generated sprite whose cookware interior is broadly translucent is invalid. Its exterior comic contour SHALL use rounded distance and antialiased fractional edge coverage at the two-final-pixel `portable-broad` candidate so the Steel set remains separated from a dark stove surface; the one-final-pixel candidate is too weak at actual map scale. The selected modern cookware approval SHALL pin the reviewed near-black contour color independently of the processor default so regeneration cannot restore the weaker Core-brown value. A hard binary square-pixel dilation, stair-stepped corner, blocky opaque fringe, or enclosed one-pixel alpha hole is invalid at source review and useful map scale. Its `CutoutComplex` mask SHALL classify every metal-bearing pot, pan, lid, and utensil surface as a continuous Stuff region. Fixed-color mask regions MAY preserve contiguous wooden handles, sanitation marks, and the exterior comic contour, but the clean 256x256 mask MUST NOT contain isolated interior fixed-color islands of 16 or fewer 4-connected opaque pixels. Tiny highlight holes, scattered one-pixel exclusions, and jagged red/black dithering inside a metal surface are invalid even when diffuse and mask alpha match.

#### Scenario: Steel cookware is viewed at useful map zooms
- **WHEN** the clean modern cookware set is rendered from Core Steel through its real finalized Def and inspected at close, ordinary, and far useful map zoom
- **THEN** its pot, pan, lids, and utensil remain one readable cookware set with smooth coherent value bands and a rounded antialiased contour rather than white pinholes, dark speckle, checkerboard edges, square pixel clusters, or stair-stepped corners
- **THEN** the complete Steel set remains visibly separated from the dark stove top instead of fading into the workstation sprite, including while held over it and while returned dirty to it
- **THEN** only the handles and dark exterior contour remain deliberately fixed-color while every metal-bearing surface receives the Steel tint continuously
...
The fallback microwave's four cardinal sprites SHALL preserve RimWorld's fixed map camera and `Graphic_Multi` cardinal-file convention rather than rotate one raster or draw four diagonal product views. Its published `interactionCellOffset` remains local south/front: the `_south` frame SHALL show the full door and controls on the screen-bottom front face, `_north` SHALL show the blank rear, `_east` SHALL be an axis-aligned vertical side view with the thin front edge at screen-right, and `_west` SHALL be an axis-aligned vertical side view with the thin front edge at screen-left. The visual correction SHALL preserve that published Def interaction offset and existing-save rotation semantics. Workshop art that depicts the fallback microwave SHALL use the reviewed `_south` front frame.

The fallback microwave SHALL read immediately as a medium-height countertop microwave rather than a VHS/VCR, drawer, wall oven, or flat electronic deck. On the 512x512 package canvas North/South SHALL have a horizontal long axis and East/West a vertical long axis, with at least a 1.10 long-to-short alpha-bounds ratio and at least 24 transparent pixels at every edge. Every frame SHALL visibly separate a broad vented top surface from a substantial near casing face. The full South door cavity SHALL have a retained width-to-height ratio between 2.0 and 3.0, a thick gasket, a separate readable control strip, and a casing depth at least half its paired top-plane depth. The door, controls, vents, feet, top plane, near casing, side/front edge, bevel, and exterior contour SHALL each be specified separately in the retained generation brief and directional approval. A diagonal diamond/three-quarter silhouette, a straight front elevation without a visible top plane, a letterbox door, a full door painted onto the top, or a cardinal frame obtained by raster rotation is invalid.
...
- **THEN** the rear, screen-right front edge, full screen-bottom front, and screen-left front edge are respectively visible while North/South remain horizontal, East/West remain vertical, and every frame retains the same fixed-camera top/near-casing projection

...
For every admitted job targeting an Immersive Chefs Processor dishwasher, the exact installed `ProcessorFramework.JobDriver_FillProcessor` and `ProcessorFramework.JobDriver_EmptyProcessor` shapes SHALL continue to own their work scans, appliance reservation, selected JobDefs, and ordinary hauling, while Immersive Chefs replaces only the stock 200-tick processor-work delay with a bounded transfer beginning no more than two engine ticks after arrival. Filling SHALL admit the carried exact dirty ware through the dishwasher's ordinary capacity, water, power, sanitation, and identity transaction. Emptying without a valid Pick Up And Haul batch SHALL immediately continue through Processor Framework's ordinary one-output selection, reservation, storage search, and haul tail. Unrelated processors and an absent, disabled, or changed exact Processor driver shape SHALL remain upstream-owned and unchanged.

When the validated public Pick Up And Haul tracker/unload shape is also active, completed output collection SHALL prefer lighter naturally completed (`ActiveProcessPercent >= 1`) output so the greatest whole-unit count fitting remaining pawn mass enters inventory. Every transferred Thing SHALL be registered at once and the upstream native unload workflow SHALL be requested once after collection. A completed partial stack MAY split only at the exact capacity boundary; its remainder SHALL stay complete in the processor. Processor Framework's `EmptyNow` completion override SHALL NOT make an active load eligible for that tracked batch; active or ruined processes and unrelated inventory SHALL remain untouched. If no natural unit fits at admission or capacity disappears between admission and appliance arrival, the job SHALL immediately use the validated one-output Processor tail with its exact fail/end lifecycle and without the 200-tick delay.
...
- **AND** each physical unit transfers within two job ticks rather than receiving a manual-washing delay
...
- **THEN** the pawn's ordinary food think tree issues the upstream one-off job without reserving ware during candidate evaluation; its accepted current driver commits the eligible cookware and plate, the pawn visibly collects and places the exact selected ingredients through the unchanged upstream toils, then collects that ware immediately before cooking with the normal speed/assistant effects, and the surviving meal receives that plate and culinary state exactly once
...
#### Scenario: Ware integration does not restart upstream ingredient collection

- **WHEN** a covered one-off self or dependent job has been admitted and another active hauling or inventory integration observes or extends the pawn's carry state
...
- **AND** the accepted current driver reserves the ware exactly once, the pawn collects it only after the final `PlaceCookIngredient` toil, returns to the selected station's interaction cell, and enters `CookMealForSelf` without an incompletable job, same-tick restart loop, duplicate reservation, or repeated ingredient selection
...
#### Scenario: Save and load during one-off cooking

- **WHEN** an exact covered CFS driver is active while the game is saved and loaded
- **THEN** the loaded job is interrupted before simulation resumes, every serialized session-marked ware unit is returned through the shared recovery path with its existing sanitation state, and no undecorated CFS cooking toil continues
...
17. Harmony; Core; XML Extensions; Ceramics (Continued); Immersive Chefs; and Gateway for exact porcelain registration, native ceramics-bench plate crafting, Stuff/color retention, and broad-`Stony` non-leakage. Processor Framework and Vanilla Expanded Framework remain absent from this minimum group because the installed provider has its own legacy fallback.
18. Harmony; Core; XML Extensions; Argonic Core; Vanilla Expanded Framework; Expanded Materials - Masonry; Immersive Chefs; and Gateway for package-ID-gated adobe registration, native crafting-spot plate production, exact brick consumption and ungraded fixed-material output without Stuff leakage.

...
- **THEN** the pawn's ordinary food think tree issues the upstream one-off job, the pawn visibly collects and places the exact selected ingredients through the unchanged upstream toils, then collects the reserved cookware and plate immediately before cooking through the upstream driver with the normal speed/assistant effects, and the surviving meal receives that plate and culinary state exactly once

...
#### Scenario: One job empties all completed outputs that fit
...
- **WHEN** the dishwasher has more completed output than the cleaner can carry without exceeding mass capacity
- **THEN** the one emptying job takes the greatest whole-unit count that fits, including a partial completed stack when applicable, and leaves every uncarried completed unit in the dishwasher
- **AND** active or ruined loads are untouched and no unit is lost, duplicated, or registered twice

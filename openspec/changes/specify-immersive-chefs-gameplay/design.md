...
Player-visible custom Things use custom art rather than renamed vanilla resources or worktables. Each distinct item/building concept receives at least two independently generated candidates. Candidate review uses alpha-clean, final-scale previews composited against retained live RimWorld map captures, and the selected asset is then rendered by the real Def in a fresh game beside representative vanilla content. Stuffable kitchenware uses a neutral value structure, explicit material-region masks, and the appropriate Stuff-capable shader so wood, stone, common metal, and luxury metal remain recognizable without baking the Stuff color into the diffuse sprite. Non-selected candidates remain ignored review inputs and do not enter release packages.

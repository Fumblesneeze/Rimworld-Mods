...
- [x] 2.17 TDD/E2E: Add optional cardinal line direction and independent exact Stuff selection to native line-shaped `Drag` steps, project both before designator preflight, and reject incompatible interactions, shapes, and empty material names.
...
- [x] 2.19 SPEC/TDD: Add a host-safe typed screenshot-mode action that toggles only RimWorld's native UI-filter state through the Unity-thread Gateway backend, records the action explicitly, and requires guaranteed restoration without process input.

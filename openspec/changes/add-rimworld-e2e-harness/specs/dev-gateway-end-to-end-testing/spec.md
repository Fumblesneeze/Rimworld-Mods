...
#### Scenario: A paused persistence action remains paused after replacement
- **WHEN** the typed save-and-reload action begins while the current game is paused
- **THEN** native pause-on-load permits only RimWorld's prescribed initialization tick, guaranteed cleanup preserves the original preference, the replacement game's tick manager is paused before the iterator resumes, and the action reports that restoration
...
#### Scenario: A short-lived external reader blocks replacement of the live E2E snapshot
- **WHEN** an external reader temporarily prevents deletion or replacement of the existing session artifact for longer than one second
- **THEN** the background persistence lane retries for a bounded window without blocking Unity's update thread
- **THEN** the previous durable snapshot remains intact until the replacement commits, and the transient lock does not open RimWorld's developer log or fail the workflow

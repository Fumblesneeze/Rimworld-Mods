...
- **THEN** preparation reads only the same item's ordered `rgFullScreenshotURLs` inventory from a no-redirect Steam Community response bounded to 2 MiB, requires an exact image count and HTTPS `images.steamusercontent.com/ugc/` URLs, replaces the complete authenticated image inventory rather than mixing sources, streams each preview under a 1 MiB ceiling, hashes those remote bytes into the baseline, and fails before mutation on missing, duplicate, non-Steam, redirected, oversized, or reordered inventory

...
The exact resolved Workshop description handed to `SetItemDescription` SHALL be valid UTF-8 without a byte-order mark or NUL. Its submitted UTF-8 bytes plus Steam's terminating NUL SHALL fit `k_cchPublishedDocumentDescriptionMax`. Checking only a shorter tokenized template is insufficient; an over-limit resolved file MUST fail during profile validation and preparation before any Steam mutation.
...
#### Scenario: A reviewed prior release does not masquerade as the new plan
- **WHEN** a new admitted update starts for the same retained Workshop item while the package durable state is `complete-reviewed` for a different valid preceding plan hash and nonzero matching item ID
- **THEN** the worker treats that terminal state only as prior-history evidence and starts the new exact-plan workflow, while a same-plan `complete-reviewed` state without its valid retained result still fails closed and a different nonterminal plan remains a conflict

#### Scenario: Steam omits authenticated additional-image URLs
- **WHEN** the authenticated SteamUGC query returns the bounded ordered additional-preview inventory but leaves one or more image URLs empty
- **THEN** preparation reads only the same item's ordered `rgFullScreenshotURLs` inventory through the reviewed ordinary browser-shaped, no-redirect Steam Community request bounded to 2 MiB, requires an exact visible image count except for the separately guarded hidden-slot scenario below and HTTPS `images.steamusercontent.com/ugc/` URLs, replaces the complete visible authenticated image inventory rather than mixing content sources, streams each preview under a 1 MiB ceiling, hashes those remote bytes into the baseline, and fails before mutation on missing, duplicate, non-Steam, redirected, oversized, or reordered inventory

#### Scenario: Steam omits an API-declared hidden additional-image slot from the Community gallery
- **WHEN** the authenticated SteamUGC inventory contains a URL-empty image slot and the Community gallery contains fewer visible images
- **THEN** preparation uses every nonempty authenticated Steam-CDN URL only as an ordered alignment anchor, requires the complete Community sequence to align unambiguously around the hidden slot, hashes every visible Community image, retains an explicit missing-content identity for the hidden slot, and fails before mutation on ambiguity, reordering, an unanchored incomplete suffix, or any origin/count/bound violation

#### Scenario: Workshop automation fails before mutation
- **WHEN** the authenticated Gateway automation returns a failed run instead of one succeeded result
- **THEN** the release worker retains a token-safe bounded rendering of that exact failed result, including its error code and message, rather than replacing the cause with a generic diagnostic before applying its bounded retry policy

#### Scenario: Existing-item update starts from a fresh clean clone
- **WHEN** the authenticated existing-item preflight matches the retained plan and the ignored release-artifact identity is absent while both the checked-in mod identity and immutable candidate identity exactly equal the declared Workshop item
- **THEN** publication atomically creates the missing durable artifact identity before dispatch, rejects any existing or source identity mismatch without overwrite, and updates only that declared item

...
- **THEN** preparation reads only the same item's ordered `rgFullScreenshotURLs` inventory through the reviewed ordinary browser-shaped, no-redirect Steam Community request bounded to 2 MiB, requires an exact image count and HTTPS `images.steamusercontent.com/ugc/` URLs, replaces the complete authenticated image inventory rather than mixing sources, streams each preview under a 1 MiB ceiling, hashes those remote bytes into the baseline, and fails before mutation on missing, duplicate, non-Steam, redirected, oversized, or reordered inventory

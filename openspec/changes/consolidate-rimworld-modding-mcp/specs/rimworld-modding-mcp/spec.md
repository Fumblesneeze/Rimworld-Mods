...
The repository SHALL contain `.codex/config.toml` configuring a required stdio MCP server with a concurrency-safe source bootstrap, Codex's logical workspace-root fallback, and bounded startup/tool timeouts. It SHALL also contain a root `.mcp.json` projection of the same bootstrap command with a project-root working directory for compatible MCP clients, while documenting that client configuration discovery is not standardized and Codex uses `.codex/config.toml`. Repository configuration MUST NOT impose an additional client approval prompt after the user has already issued an explicit order whose material scope matches the typed operation; the active client/agent authorization policy remains authoritative, and an absent or materially different order does not authorize invocation. Operation-level exact-plan admission, ownership, validation, recovery, and evidence controls MUST apply independently of client prompt behavior. The bootstrap MUST expose no repository operations of its own, MUST reserve stdout for the executed MCP server, and MUST use a bounded cross-process lease plus isolated staging and verified immutable output so cold, concurrent, failed, or cancelled starts cannot race shared build files or publish a partial executable. The server MUST validate that its configured root contains the expected repository markers and MUST NOT search upward into or operate on another repository.
...
#### Scenario: An explicit operation order is not reconfirmed
- **WHEN** the user has already ordered an external or local mutation and the prepared typed operation matches that order's target and material scope
- **THEN** repository MCP configuration adds no second approval prompt, while the operation still enforces its exact admission, ownership, validation, recovery, and evidence boundaries

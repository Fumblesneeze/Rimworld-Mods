...
Circinus' attributed per-mod/per-patch time is gross execution cost, not a complete causal net-impact measurement. A skip-capable prefix can replace vanilla work, two targets may share one patch method, and another mod or a larger colony can induce work inside the measured code. The repository therefore keeps gross attribution, profiler overhead, separately reported product-present/product-absent samples, and version-over-version regression as distinct claims.
...
Each ordinary comparison ID needs exactly one instrumented workload. It may additionally own a complete profiler-overhead calibration trio—instrumented, armed-disabled-wrapper, and fully disarmed—whose workload/selector/checkpoint metadata plus staging owner must match. Partial calibration pairs are rejected, but the two disabled lenses are not required for ordinary historical runs. A product-absent benchmark is an explicit independent diagnostic: it may reuse one Gateway-owned, product-reference-free fixture implementation while changing only the exact active product package, but it is not coupled into the ordinary plan or canonical baseline as a synthetic net-control claim. Product-owned feature fixtures report gross attribution and historical regression.
...
The ordinary historical benchmark is the Circinus-instrumented representative workload, repeated in fresh processes and summarized as a distribution. Three additional comparisons remain available when a maintainer explicitly calibrates overhead or passive load-time impact:
...
- a separately declared product-absent control remains an independent stochastic diagnostic; canonical reports retain both source distributions and do not pair repetitions by process order.

Instrumented minus armed-disabled estimates active method-timing/sampling overhead; armed-disabled minus fully disarmed estimates wrapper overhead; instrumented minus fully disarmed estimates total method-instrumentation overhead. Circinus recorder/sample collection remains common to all three, so none is mislabeled as analyzer-absent. These are occasional noisy calibration runs, not a mandatory multiplier on every optional-mod benchmark.

An Immersive Chefs-specific cooking/dishwashing workload is not falsely subtracted from vanilla when the absent product cannot perform those jobs. A smaller Gateway-owned neutral colony fixture may be run repeatedly with and without Immersive Chefs as two separate diagnostics, but the canonical runner retains their independent distributions rather than constructing a delta. The neutral scenario uses ordinary pawn, animal, need, health, map, Harmony, and mod-component ticking. It does not pin long-lived jobs, random state, IDs, positions, inventories, or terminal state. Product-owned feature workloads remain present-only and measure historical feature-path trends.

### 5. The benchmark lifecycle has explicit calibrated phases

Each scenario declares a workload version, warm-up ticks, sample ticks, game speed, and repetition count. Defaults are stored in the scenario source and may be changed only as a versioned workload change; the CLI may override them and records every override. Three fresh repetitions are the ordinary minimum. RimWorld's global random state and unique-ID manager are never seeded or reset by a performance fixture.
...
3. arrange the broad fixture while paused and warm it through ordinary frames/ticks with Circinus recording disabled, allowing normal AI, pathing, random events, and state changes;
4. resolve and arm the exact hand-armed method set, reset only benchmark-owned measurement counters after warm-up, start one labeled Circinus run, add a sample-start marker, and enable the profiler;
...
The initial workload creates a representative multi-room colony with long corridors and separated storage/service areas; about 36 human pawns distributed among cooking, assistance, cleaning/hauling, nursing/patient, and ordinary dining roles; about 24 animals; multiple simultaneous simple/fine/lavish cooking bills; prep and four linked support station types; domestic and industrial dishwashers; microwaves; dirty/clean stockpiles; several patients and nurses; guest/service activity when the relevant package is active; and two world caravans with pawns, animals, meals, plates, and cutlery. A versioned scenario manifest records its broad counts, facilities, package group, duration, and required activity. Coarse health checks invalidate a broken or idle run; ordinary identities, jobs, paths, inventories, and terminal layout are expected to differ.

Optional-mod variants add only behavior owned by their exact group: Processor/Dubs dishwashing; Gastronomy/Hospitality service with Common Sense ownership arbitration; and a variety/VNPE/material/DLC branch covering public ingredient provenance, modded metal kitchenware, Royalty and Biotech. The all-supported performance matrix is the dependency-safe union of those branches, not the much broader release startup canary and not every downloaded food ecosystem. Each family has an ordinary repeated instrumented declaration and may retain on-demand overhead-control declarations. The base workload remains useful with only Harmony, Core, Circinus, XML Extensions, Immersive Chefs, and Gateway.

### 7. Historical comparison is explicit and reviewable

Each report writes untouched Circinus JSON, normalized JSON, CSV, and a human Markdown summary. Every metric is labeled as gross attribution, instrumentation overhead, or version trend. A baseline is an immutable reviewed report named by workload version, game version, exact package order, product commit/build hash, Circinus assembly/schema/profiling-policy identity, evidence lens, relevant hardware/runtime fingerprint, duration, and repetition policy. For every metric it retains each raw repetition plus sample count, arithmetic mean, minimum, maximum, and sample standard deviation. The runner compares only compatible broad identities unless the caller explicitly requests a cross-version informational diff; random seeds, generated IDs, and exact world-state hashes are never compatibility fields. Product-present and product-absent workloads remain separately reported independent samples; canonical reports do not pair them by process order or invent a matched delta distribution.
...
- **Gross patch time is mistaken for net mod impact** → Label gross/shared/skip-capable/ambiguous costs, keep optional product-absent samples separate, and never pair independent stochastic repetitions as though they were matched observations.
...
- **A fast result is caused by stalled gameplay** → Assert coarse actor/building/caravan/job throughput and absence of errors before accepting each sample.
- **Ordinary game randomness produces variance** → Retain every healthy repetition, report mean/min/max/sample-standard-deviation, increase repetitions when the spread is too noisy, and never mutate RimWorld's global RNG or ID manager to make samples match.

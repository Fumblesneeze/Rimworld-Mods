...
Optional-mod variants add only behavior owned by their exact group: Processor/Dubs dishwashing; Gastronomy/Hospitality service with Common Sense ownership arbitration; and a variety/VNPE/material/DLC branch covering public ingredient provenance, modded metal kitchenware, Royalty and Biotech. The all-supported performance matrix is the dependency-safe union of those branches, not the much broader release startup canary and not every downloaded food ecosystem. Each family has instrumented, armed-disabled and fully-disarmed declarations with identical branch-specific checkpoints. The base workload remains useful with only Harmony, Core, Circinus, Immersive Chefs, and Gateway.

...
- [ ] 25.6 TDD RED/GREEN: Reproduce a transient Windows lock on the owned run manifest during stopped-tombstone replacement, retry for a short bounded interval, and retain fail-closed retryable ownership for a persistent lock.
- [ ] 25.7 REGRESSION/REVIEW: Run focused and full Gateway tests, independently review the shutdown hardening, and repeat the rejected current-build cutlery-item observation through a clean exact-PID shutdown.
